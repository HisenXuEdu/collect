...
def record_accl(host):
    channel=1
    dev_emg = pytrigno.TrignoAccel(channel_range=(0,channel-1), samples_per_read=100,
                        host=host)
    dev_emg.start()
    time_emg_begin = time.time()
    print('IMU开始时间：',time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time_emg_begin)))
    ...
    data_EMG.to_csv('./Data/emg/IMU'+str(int(time_emg_begin*1000))+'.csv', index=None)
